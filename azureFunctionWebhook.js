...
async function analyzeTextWithOpenAI(text, cefrLevel = "B1", orgPrompts = {}) {
  try {
    ...
    // context.log(`[analyzeTextWithOpenAI] Error: ${error.message}`);
    return [];
  }
}
...
async function generatePronunciationChallenge(transcript, orgPrompts = {}) {
  ...
  try {
    ...
    // context.log(`[generatePronunciationChallenge Error] ${error.message}`);
    return "";
  }
}
...
async function Vocabulary_Booster(text, orgPrompts = {}) {
  try {
    ...
    try {
      ...
      //   context.log("Error parsing Vocabulary Booster response:", jsonError);
      return [];
    }
  } catch (error) {
    // context.log(`[Vocabulary_Booster Error] ${error.message}`);
    return [];
  }
}
...
- Include 5-8 items per exercise`;
  ...
  try {
    ...
    // context.log(`[generateMCQs Error] ${error.message}`);
    return "";
  }
}
...
async function generateAIAnalysis(payload) {
  ...
  //   context.log("ai processing started for ", name);
  ...
  try {
    ...
    // context.log("Error setting assessment to pending:", err);
  }
  ...
  if (orgPrompts.ENABLE_ANALYZE_TEXT_WITH_OPENAI) {
    try {
      ...
      //   context.log("Error in OpenAI analysis:", error);
  ...
  if (orgPrompts.ENABLE_PRONUNCIATION_CHALLENGE) {
    try {
      ...
      //   context.log("Error in pronunciation challenge:", error);
  ...
  if (orgPrompts.ENABLE_COACHING_SPACE) {
    try {
      ...
      //   context.log("Error in coaching space:", error);
  ...
  if (orgPrompts.ENABLE_VOCABULARY_BOOSTER) {
    try {
      ...
      //   context.log("Error in vocabulary booster:", error);
  ...
  if (orgPrompts.ENABLE_GENERATE_MCQS) {
    try {
      ...
      //   context.log("Error in MCQ generation:", error);
  ...
  //   context.log("ai data: ", JSON.stringify(speakerAssessment));
  ...
  try {
    ...
    //   status: "completed",
    // });
    // context.log("Saved assessment to table for ", id);
    try {
      let entity;
      try {
    ...
    //   status: "completed",
    // });
    // context.log("Error saving assessment to Azure Table:", err);
...
module.exports = async function (context, myQueueItem) {
  ...
  try {
    ...
    if (!transcriptUrl || !organization) {
      context.log("Missing transcriptUrl or organization");
      return;
    }

    const res = await fetchFn(transcriptUrl, {
      headers: {
    ...
    if (!res.ok) {
      context.log("Transcript download failed:", res.status, res.statusText);
      return;
    }
    ...
    if (orgTokens < mappedUsersOnly.length) {
      context.log(
        `❌ Not enough tokens for organization: ${organization}. Required: ${mappedUsersOnly.length}, Available: ${orgTokens}. Skipping auto-report.`,
      );
      return;
    }

    // Generate AI reports in parallel and store in DB
    await Promise.all(
      mappedUsersOnly.map(async (user) => {
